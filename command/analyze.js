...
const analyzeCommand = new Command("analyze").description(
  "analyze: analyze the register user assests",
);
...
analyzeCommand.addArgument(file).action(async (file, options) => {
  ...
  if (file != null && fs.existsSync(file)) {
    whiteList = JSON.parse(fs.readFileSync(path, "utf8"));
  }
